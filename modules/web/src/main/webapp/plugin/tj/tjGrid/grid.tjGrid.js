(function($) {
	...
	function TjGrid(target,options){
		...
		this.viewDetail = $.extend({view:true}, options.viewDetail || {});
		...
		this.dynamicColumns = options.dynamicColumns==undefined?false:options.dynamicColumns;//动态增减列的功能，默认关闭
	...
	$.extend(TjGrid.prototype, {
		...
		relateAjaxFun: function(obj,type) {
			var ajaxInit = {
					url:obj.url + '?rd=' + Math.random(),
				    type:'post',
				    async:false,
				    dataType:'json',
					success:function(data){
						$('*[id="'+obj.relateName+'"]').empty();
						if (data.result == true){
							if (data.data != ''){
			...
			var relatedVal = $('*[id="'+obj.relatedName+'"]').val();
			if ($('*[id="'+obj.relatedName+'"]')[0].tagName == 'SELECT'){
				ajaxInit.data={id:relatedVal};
			}else if ($('*[id="'+obj.relatedName+'"]')[0].tagName == 'INPUT'){
				ajaxInit.data={name:relatedVal};
			}
			$.ajax(ajaxInit);
		},
		getLoadUrl:function(){
		...
		getDynamicColumnsAjaxUrl:function(){
			var thisUrl = window.location.href;
			var dynamicColumnsAjaxUrl = thisUrl.substring(0,thisUrl.lastIndexOf('!')) + '!dynamicColumnsAjax.ajax';
			return dynamicColumnsAjaxUrl;
		}
	});
	...
	function TjGridView(tjGrid,setting) {
		...
		this.dynamicColumnsDivId='grid-dynamic-' + id;
	...
	$.extend(TjGridView.prototype, {
		...
		getcolModelsForJqGrid:function(){
			var colModels=[];
			//预先初始化动态增减列
			if (this.tjGrid.dynamicColumns){
				var dynamicColumnsMap = eval(this.tjGrid.dynamicColumns);
				$.each(this.tjGrid.tjColModel,function(index, value){
					var isHidden = false;
					if (value.options.hidden){
						isHidden = true;
					}else if (dynamicColumnsMap[value.options.name] == undefined){
						isHidden = true;
					}
					var colModel = {name:value.options.name,
	    ...
	    bindRelateSelect:function(){
			...
			$.each(this.tjGrid.relateArray, function(i, obj){
				//在自动完成的被级联输入框获得焦点时先记录下当前取值，在失去焦点时进行比较，如果改变才触发级联操作
				$('*[id="'+obj.relatedName+'"]').blur(function(event){
					//如果是自动完成，并且以完成选择时（即当前鼠标的焦点不在自动完成下拉列表中，而在其他页面元素中的时候），才执行级联
					if ($(event.currentTarget)[0].tagName == 'INPUT' && !($(document.activeElement).hasClass('ac_results'))){
						if ($(this).val() != $(event.currentTarget).data('s')){
							$this.tjGrid.relateAjaxFun(obj);
							$(event.currentTarget).data('editRelatedVal', $(this).val());
						}
					}else if ($(event.currentTarget)[0].tagName == 'SELECT'){
						$this.tjGrid.relateAjaxFun(obj);
					}
				});
			});
		...
		bindEvent:function(){
	    	...
	    	$(window).bind('resize', function() {
				$('#'+$this.gridTableId).setGridWidth($(window).width()-30); 
			}).trigger('resize');
	    		...
	    		$('#'+this.gridTableId).jqGrid('navButtonAdd','#'+this.pageBarId,{caption:"查询",title:"分类查询", buttonicon :'ui-icon-pin-s',
		    		onClickButton:function(){
		    			...
		    			$('input[id^="gs"]').each(function(){
				    		...
				    		if (attrStr != undefined && (attrStr.indexOf('date') != -1 || attrStr.indexOf('time') != -1 || attrStr.indexOf('dateYM') != -1)){
...
//						});
						...
						if ($this.fillDefaultValues($this.tjGrid.tjColModel,$this) == 'success'){
							...
							$.each($this.tjGrid.tjColModel,function(index, value){
								...
								if (value.options.related != ''){
									$('*[id="'+name+'"]').data('editRelatedVal', editValue);
								}
								if (value.options.inputType == 'input' || value.options.inputType == 'date' || value.options.inputType == 'datetime'|| value.options.inputType == 'dateYM'){
									$('*[id="'+name+'"]').val(editValue);
								} else if (value.options.inputType == 'select') {
									$('*[id="'+name+'"]').find('option').each(function(){
										if ($(this).text() == editValue){
								...
								}else if (value.options.inputType == 'auto'){
									$('*[id="'+name+'"]').val(editValue);
			...
			if ($this.tjGrid.inputUrl.content != ''){
				if ($this.tjGrid.inputUrl.type == ''){
					...
					$('#'+this.editId).before('<a ztype="popUp" title="编辑'+$this.tjGrid.name+'" url="'+$this.tjGrid.inputUrl.content+'"></a>');
	    ...
	    //动态添加列初始化
	    createDynamicColumnsDiv:function(){
	    	var $this = this;
	    	$('#jqgh_' + this.gridTableId + '_rn').css('background','url("'+ctp+'/images/style1/ico10.png") no-repeat left top');
	    	var html = '<div id="'+this.dynamicColumnsDivId+'"><table>';
	    	$.each(this.tjGrid.tjColModel,function(i,n){
	    		if (i%2 == 0){
	    			html += '<tr>';
	    		}
	    	...
	    	html+= '</table></div>';
	    	$(this.tjGrid.target).after(html);
	    	$('#'+this.dynamicColumnsDivId).dialog({
				height: 'auto',
				width: 200,
				autoOpen:false,
				resizable: false,
					...
					"确定": function() {
	    				...
	    				$('input[name="dynamicCheckbox"]:checked').each(function(){
	    					...
	    					if (tempVal.indexOf('.') != -1){
	    						dynamicColumnsResult += tempVal.substring(0, tempVal.indexOf('.')) + ',';
	    					}else {
	    						dynamicColumnsResult += tempVal + ',';
	    					}
	    				});
	    				$.ajax({
	    					url: $this.tjGrid.getDynamicColumnsAjaxUrl(),
	 				   		data: {dynamicColumnsResult : dynamicColumnsResult.substring(0, dynamicColumnsResult.length - 1)},
 				   			dataType: 'json'
	    				});
						$('#'+$this.dynamicColumnsDivId).dialog('close');
					},
					"关闭":function(){
						$('#'+$this.dynamicColumnsDivId).dialog('close');
					}
	    	...
	    	$.each(this.tjGrid.dynamicColumns,function(i,n){
	    		$('input[name="dynamicCheckbox"][value="'+i+'"]').attr('checked','checked');
	    	});
	    	$('#jqgh_' + this.gridTableId + '_rn').click(function(){
	    		var top = $(this).offset().top - $(document).scrollTop();
				var left = parseInt($(this).offset().left) + parseInt($(this).width()) + 5;
	    		$('#'+$this.dynamicColumnsDivId).dialog('option', 'position', [left,top] );
	    		$('#'+$this.dynamicColumnsDivId).dialog('open');
	    	});
	    	$('input[name="dynamicCheckbox"]').click(function(){
	    		if ($(this).attr('checked') == 'checked'){
	    			//显示列
	    			$('#'+$this.gridTableId).jqGrid('showCol',$(this).val());
	    		}else {
	    			//隐藏列
	    			$('#'+$this.gridTableId).jqGrid('hideCol',$(this).val());
	    		}
	    		$('#'+$this.gridTableId).setGridWidth($(window).width()-37); 
	    	});
	    },
	    createDetail: function(){
	    	var html ='<div id="'+this.detailId+'" style="display:none;" class="detailtable">';
	    	html +='<div style="text-align:center;"><table class="infotable_d" width="100%" border="0" cellpadding="0" cellspacing="0">';
	    	var $this=this;
	    	id=$('#'+$this.gridTableId).jqGrid('getGridParam','selrow');
	    	var val=$('#'+$this.gridTableId).getRowData(id);
	    	$.each(this.tjGrid.tjColModel,function(index, value){
	    		html += '<tr><td class="titletd" style="text-align:center;">' + value.options.header +'</td>';
				var result;
				for(var p in val){
					if(p==value.options.name){
						result=val[p];
				...
				if (result == ''){
					html += '<td id="'+$this.detailTdPrefix+value.options.name+'" class="td" style="text-align:center;">&nbsp;</td></tr>';
				}else {
					html += '<td id="'+$this.detailTdPrefix+value.options.name+'" class="td" style="text-align:center;">'+result+'</td></tr>';
				}
	    	});
			html+='</table></div>';
	    	html+='<div class="form_submitDiv">';
			html+='<input id="'+this.detailCloseId+'" type="button" value="关闭" />';
			html+='</div></div>';
			return html;
	    },
...
//	    	return html;
		...
		createInput: function(){
	    	...
	    	html +='<table class="infotable" width="100%" border="0" cellpadding="0" cellspacing="0">';
	    	var $this=this;
	    	var modelName = this.tjGrid.id.substring(0,this.tjGrid.id.indexOf('-'));
	    	$.each(this.tjGrid.tjColModel,function(index, value){
	    		if (value.options.inputType == 'checkbox' || value.options.inputType == 'select'){
		    	...
		    	html+='<td class="td1" style="width:120px;"><label for="'+value.options.name+'">'+value.options.header+'：</label></td><td>';
			...
			html+='<input type="button" id="'+this.closeId+'" value="关闭" />';
			html+='</div>';
			html+='</form>';
			html+='</div>';
			return html;
	    ...
	    createEachElement:function(value,$this,multi){
	    	...
	    	if (value.options.editable){
	    		if (value.options.inputType == 'input'){
		    		...
		    		//下拉选形式
		    		html += '<select id="'+$this.inputPrefix+value.options.name+multi+'" style="width:auto;" ';
	    			var submitName = value.options.name;
	    			var index = submitName.indexOf('.');
	    			var name = '';
	    			if (index == -1){
	    				...
	    				if (submitName.substring(index + 1) == 'codeDesc'){
	    					html += 'name="'+name+'.paraCode" ';
	    				}else {
	    					html += 'name="'+name+'.id" ';
	    				}
	    			}
		    		...
		    		html += '<input id="'+$this.inputPrefix+value.options.name+multi+'" ztype="auto"';
		    		html += ' name="'+value.options.name+'" ';
		    		if (value.options.verify != null){
		    			html += ' verify="'+value.options.header+'|'+value.options.verify + '"';
		    		}
		    		if (value.options.zurl != undefined && value.options.zurl != ''){
		    			html += ' zurl="' + value.options.zurl + '"';
		    		}
		    		html += '/>';
		    	}else if (value.options.inputType == 'date' || value.options.inputType == 'datetime'|| value.options.inputType == 'dateYM'){
		    		html += '<input id="'+$this.inputPrefix+value.options.name+multi+'" ztype="'+value.options.inputType+'"';
		    		html += ' name="'+value.options.name+'" ';
		    		if (value.options.verify != null){
		    			html += ' verify="'+value.options.header+'|'+value.options.verify + '"';
		    		}
	    ...
	    fillDefaultValues:function(tjColModel,$this){
	    	...
	    	$.each(tjColModel,function(index, value){
				...
				if (value.options.inputType == 'input' || value.options.inputType == 'textarea'){
					...
					var items = eval(value.options.items);
					var html = '';
    				if (value.options.verify != 'NotNull'){
    					html += '<option value="-1" selected="selected">无</option>';
    				}else {
    					if (items.length == 0){
